...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='matlab_ros2_tcp_bridge_pkg').find('matlab_ros2_tcp_bridge_pkg')
    default_model_path = os.path.join(pkg_share, 'urdf/mobile2_bot.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/mobile2_bot/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'mobile2_bot', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
